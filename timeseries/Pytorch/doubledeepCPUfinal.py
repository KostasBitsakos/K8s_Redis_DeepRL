...
def plot_metrics(episode_loads, episode_mem_usages, episode_cpu_usages, episode_latencies, episode_num_vms, output_folder):
    ...
    for episode_load in episode_loads:
        ax1.plot(range(len(episode_load)), episode_load, color=metric_colors['Load'], alpha=0.5, label='Load')
    ax1.set_xlabel('Time')
    ax1.set_ylabel('Load')
    ...
    for episode_num_vm in episode_num_vms:
        ax1b.scatter(range(len(episode_num_vm)), episode_num_vm, color=metric_colors['VMs'], marker='o', alpha=0.3, label='VMs')
    ax1b.set_ylabel('Number of VMs')

    # Plot Memory Usage with VMs
    ax2 = plt.subplot(2, 2, 2)
    for episode_mem in episode_mem_usages:
        ax2.plot(range(len(episode_mem)), episode_mem, color=metric_colors['Memory Usage'], alpha=0.5, label='Memory Usage')
    ax2.set_xlabel('Time')
    ...
    ax2b = ax2.twinx()
    for episode_num_vm in episode_num_vms:
        ax2b.scatter(range(len(episode_num_vm)), episode_num_vm, color=metric_colors['VMs'], marker='o', alpha=0.3, label='VMs')
    ax2b.set_ylabel('Number of VMs')

    # Plot CPU Usage with VMs
    ax3 = plt.subplot(2, 2, 3)
    for episode_cpu in episode_cpu_usages:
        ax3.plot(range(len(episode_cpu)), episode_cpu, color=metric_colors['CPU Usage'], alpha=0.5, label='CPU Usage')
    ax3.set_xlabel('Time')
    ...
    ax3b = ax3.twinx()
    for episode_num_vm in episode_num_vms:
        ax3b.scatter(range(len(episode_num_vm)), episode_num_vm, color=metric_colors['VMs'], marker='o', alpha=0.3, label='VMs')
    ax3b.set_ylabel('Number of VMs')

    # Plot Latency with VMs
    ax4 = plt.subplot(2, 2, 4)
    for episode_latency in episode_latencies:
        ax4.plot(range(len(episode_latency)), episode_latency, color=metric_colors['Latency'], alpha=0.5, label='Latency')
    ax4.set_xlabel('Time')
    ...
    ax4b = ax4.twinx()
    for episode_num_vm in episode_num_vms:
        ax4b.scatter(range(len(episode_num_vm)), episode_num_vm, color=metric_colors['VMs'], marker='o', alpha=0.3, label='VMs')
...
if __name__ == "__main__":
    ...
    print("✅ All evaluations completed! Results are in 'final_metrics'.")

...
def train_agent(train_steps, max_episodes_per_step=1000):
    ...
    for _ in range(train_steps):
        ...
        while not done:
            for episode_idx in tqdm(range(max_episodes_per_step)):
                ...
                episode_read_percentage.append(next_state[1])
                episode_mem_usage.append(next_state[2])
                episode_cpu_usage.append(next_state[3])
                episode_latency.append(next_state[4])
                episode_num_vm.append(next_state[5])
    ...
    plt.legend()
    plt.savefig(f'training_rewardsCPU_{train_steps}.jpg')
    plt.close()
    with open(f"rewards2_{train_steps}.txt", "w") as f:
        for reward in rewards:
            f.write(str(reward) + "\n")

    agent.save_model(f'modelCPU_{train_steps}.pth')
...
def plot_metrics(episode_loads, episode_read_percentages, episode_mem_usages, episode_cpu_usages, episode_latencies, episode_num_vms, output_filename_prefix):
    ...
    plt.tight_layout()
    plt.savefig(f'{output_filename_prefix}_metricsCPU.jpg')
    plt.close()

if __name__ == '__main__':
    train_steps_list = [1, 2, 5, 10, 20]
    eval_steps = 4000
    ...
    for train_steps in train_steps_list:
        ...
        model_filename = f'modelCPU_{train_steps}.pth'
        ...
        output_prefix = f'evaluation_plots_modelCPU_{train_steps}'
